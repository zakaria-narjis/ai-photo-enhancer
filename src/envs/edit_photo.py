...
class AdjustHighlights:
    ...
    def __call__(self, list_hsv, parameters):
        ...
        return [
            list_hsv[0],
            list_hsv[1],
            1 - (1 - v) * (1 - hilights_mask * hilights * 5),
        ]
...
class AdjustVibrance:
    ...
    def __call__(self, list_hsv, parameters):
        ...
        return [
            list_hsv[0],
            s * vibrance * vibrance_flg + s * (1 - vibrance_flg),
            list_hsv[2],
        ]
...
class Hsv2Bgr:
    ...
    def __call__(self, list_hsv, parameters):
        h, s, v = list_hsv
        h = (
            h * relu(np.sign(h - 0)) * (1 - relu(np.sign(h - 360)))
            + (h - 360) * relu(np.sign(h - 360)) * (1 - relu(np.sign(h - 720)))
