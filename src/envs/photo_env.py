...
class PhotoEnhancementEnv(gym.Env):
    ...
    def __init__(
        ...
        if self.use_txt_features == "embedded":
            ...
            self.observation_space = Observation_Space(
                shape=(
                    self.batch_size,
                    self.features_size + 16,
                ),  # 16 is the number of txt features
                dtype=torch.float32,
            )
            ...
            self.observation_space = Observation_Space(
                shape=(
                    self.batch_size,
                    self.features_size + (64 * 3),
                ),  # 64*3 is the number of histogram features
                dtype=torch.float32,
            )
        ...
        self.target_images = (
            None  # Batch of images (B,3,H,W) of target images (ground_truth)
        )
    ...
    def compute_preprocessor_threshold(
        ...
        with torch.no_grad():
            ...
            pre_enhanced_image = self.preprocessor_photo_editor(
                self.state["source_image"].permute(0, 2, 3, 1),
                pre_batch_actions,
            )
            pre_enhanced_image = pre_enhanced_image.permute(0, 3, 1, 2)
            pre_rewards = self.compute_rewards(
                pre_enhanced_image, self.state["target_image"]
            )
    ...
    def reset(self):
        ...
        if self.use_txt_features == "embedded":
            (
                source_image,
                txt_semantic_features,
                img_semantic_features,
                target_image,
    ...
    def step(self, batch_actions: torch.Tensor):
        ...
        if self.use_txt_features == "embedded":
            ...
            enhanced_image = self.photo_editor(
                source_images.permute(0, 2, 3, 1), batch_actions
            )
