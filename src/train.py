...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=str,
        help="directory to put experiment results",
        default=os.path.join(current_dir.parent, "experiments/runs"),
    )
    ...
    try:
        ...
        if env_config.preprocessor_agent_path is not None:  # Double agent mode
            ...
            agent.backbone.model.load_state_dict(
                env.preprocessor_agent.backbone.model.state_dict()
            )
        ...
        for i in tqdm(
            ...
            while True:
                ...
                if (
                    ...
                    break
            if (
                agent.global_step % 200 == 0
                and agent.global_step >= agent.args.learning_starts
            ):
                agent.backbone.eval().requires_grad_(False)
                ...
                with torch.no_grad():
                    ...
                    if env_config.preprocessor_agent_path is not None:
                        ...
                        agent.writer.add_images(
                            "test_images",
                            test_env.state["source_image"][:n_images],
                            1,
                        )
                        agent.writer.add_images(
                            "test_images",
                            test_env.state["enhanced_image"][:n_images],
                            2,
                        )
                        agent.writer.add_images(
                            "test_images",
                            test_env.state["target_image"][:n_images],
                            3,
                        )
                    else:
                        agent.writer.add_images(
